...
static void ch347_write_bulk_callback(struct urb *urb)
{
	...
	/*
	 * This is the only place a submitted TX buffer is returned to the pool.
	 * The buffer backs an in-flight DMA transfer until the URB completes,
	 * so ch347_data_xfer() must not release it earlier.
	 */
